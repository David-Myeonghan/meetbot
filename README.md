...
설계 답안(스펙)이 요구하지만 이 데모에는 아직 없는 것 — 정직하게 표기:

- **참석자 거절 감지 → 주최자 재조율 카드**: 거절 이벤트 수신에 캘린더 push 알림(webhook) 구독이 필요 — 데모는 Socket Mode 로컬이라 공개 URL이 없어 보류
- **버튼 클릭 시점 stale 카드 재조회**: 봇 외부에서 바뀐 회의를 카드가 모르는 문제 — 취소·이동 실패 시 에러 카드로만 처리 중
- **필수/선택 참석자 구분**: 폼·엔진 모두 전원 필수로 취급

구현된 스펙 안전장치: 생성 직후 회의실 수락 상태 재조회(사후 자동 거절 감지 → 주최자 알림 + room_declined 감사 기록)는 포함.
